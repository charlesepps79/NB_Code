data _null_;
	call symput("auditnbcc",
		"WORK.'118306A_RMC_NB_6.0_Final_Mailfil'n");
	call symput("auditfile",
		"\\mktg-app01\E\Production\Audits\NBCC AUDIT - 6.0 - Final Mail File.xlsx");
run;
...
proc import 
	datafile = "\\mktg-app01\E\Production\Master Files and Instructions\AMTID Master_UPDATE.xlsx" 
	dbms = excel out = amtids replace;
run;
...
proc tabulate 
	...
	var fico tu_variable7 equ_attribute_9 equ_NumOpenMortAccts
		tu_scrfi01 tu_mtn001;
	tables lettercd * state * checkamount, 
		n fico * min * f = 5.0 
		  fico * max * f = 5.0 
...
proc tabulate 
	data = auditnbcc;
	class node_code;
	var fico equ_attribute_7 tu_variable10 equ_ATTRIBUTE_9
		tu_variable7;
	tables node_code, n fico * Min * f = 5.0 
						fico * max * f = 5.0 
