data _null_;
	*** BranchNumber, Checknumber, SS7 as string, amt_id as # ---- ***;
	call symput("auditnbcc",
		"WORK.'119506A_RMC_NB_7.0_FINAL_MA_0000'n");
	call symput("auditfile",
		"\\mktg-app01\E\Production\Audits\NBCC AUDIT - 7.0 - Final Mail File.xlsx");
run;
...
data auditnbcc;
	...
	if check_routing_number_x = check_routing_number 
		then RoutingNum_Error = 0;
...
ods excel file = "&auditfile" options(sheet_name = "Data Snippet" 
									  sheet_interval = "none");

proc summary 
	data = auditnbcc print;
run;
...
ods select all;
ods excel options(sheet_name = "Credit Score Info" 
				  sheet_interval = "none" );

proc means 
	data = auditnbcc n min max maxdec = 0;
	var fico Vantage;
run;
...
proc tabulate 
	...
	tables filecode all, n equ_BNI_SCORE * min * f = 5.0 
						   equ_BNI_SCORE * max * f = 5.0 
						   tu_score_Attribute * min * f = 5.0 
						   tu_score_Attribute * max * f = 5.0;
run;
...
ods select all;
ods excel options(sheet_name = "Offer Amount by CreditSc" 
				  sheet_interval = "none");
...
proc tabulate 
	...
	var fico 
		tu_variable7 
		equ_attribute_9 
		equ_NumOpenMortAccts 
		tu_scrfi01 
		tu_mtn001;
tables lettercd * state * checkamount,n 
	fico * min * f = 5.0 
	fico * max * f = 5.0 
	tu_variable7 * min * f = 5.0 
	tu_variable7 * max * f = 5.0 
	equ_attribute_9 * min * f = 5.0 
	equ_ATTRIBUTE_9 * max * f = 5.0 
	equ_NumOpenMortAccts * min * f = 5.0 
	equ_NumOpenMortAccts * max * f = 5.0 
	tu_scrfi01 * min * f = 5.0 
	tu_scrfi01 * max * f = 5.0 
	tu_mtn001 * min * f = 5.0 
	tu_mtn001 * max * f = 5.0;
run;
...
ods select all;
ods excel options(sheet_name = "Campaign Info" 
				  sheet_interval = "none");
...
proc tabulate 
	...
	var CheckAmount;
	tables CheckAmount_SpelledOut, 
		CheckAmount * mean * f = dollar10.2;
run;
...
proc tabulate 
	...
	tables node_code,n 
		fico * Min * f = 5.0 
		fico * max * f = 5.0 
		equ_ATTRIBUTE_7 * min * f = 5.0 
		equ_attribute_7 * max * f = 5.0 
		tu_Variable10 * min * f = 5.0 
		tu_variable10 * max * f = 5.0 
		equ_ATTRIBUTE_9 * min * f = 5.0 
		equ_attribute_9 * max * f = 5.0 
		tu_variable7 * min * f = 5.0 
		tu_variable7 * max * f = 5.0;
	label equ_attribute_7 = CollEQX 
		  tu_variable10 = CollTU 
		  equ_attribute_9 = OpenEQX 
		  tu_variable7 = OpenTU;
run;
...
ods select all;
ods excel options(sheet_name = "FNAME and MNAME check" 
				  sheet_interval = "none");
...
ods select all;
ods excel options(sheet_name = "Branch Info Check" 
				  sheet_interval = "none");
